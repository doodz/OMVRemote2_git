...
public class StatisticsScrollingActivity extends AppCompatBaseActivity implements OutputListener, NoticeDialogListener {
    ...
    private NutController mNutController = new NutController(this);
    ...
    private Boolean Nutplugin = false;
...
*/
    ...
    private void AddSensors(SensorsSettings settings2) {
        ...
        if (settings2.getCpuenable()) {
            ...
            //Cpu Temps
            if (settings.getBoolean("swithSensorsCpuHour", true)) {
            ...
            if (settings.getBoolean("swithSensorsCpuHourMonth", true)) {
                item = new StatisticItem(DaignostiquesSystemeController.Diagnostique_sensors_month);
                mStatisticItemList.add(item);
            }
            if (settings.getBoolean("swithSensorsCpuHourYear", true)) {
        ...
        if (settings2.getMbtemp()) {
            ...
            if (settings.getBoolean("swithMotherBoardHour", true)) {
                item = new StatisticItem(DaignostiquesSystemeController.Diagnostique_mb_hour);
                mStatisticItemList.add(item);
            }
            if (settings.getBoolean("swithMotherBoarDay", true)) {
            ...
            if (settings.getBoolean("swithMotherBoardMonth", true)) {
                item = new StatisticItem(DaignostiquesSystemeController.Diagnostique_mb_month);
                mStatisticItemList.add(item);
            }
            if (settings.getBoolean("swithMotherBoardYear", true)) {
        ...
        if (settings.getBoolean("swithCPUFanSpeedHour", true)) {
            item = new StatisticItem(DaignostiquesSystemeController.Diagnostique_fan_hour);
            mStatisticItemList.add(item);
        }
        if (settings.getBoolean("swithCPUFanSpeedDay", true)) {
        ...
        if (settings.getBoolean("swithCPUFanSpeedMonth", true)) {
            item = new StatisticItem(DaignostiquesSystemeController.Diagnostique_fan_month);
            mStatisticItemList.add(item);
        }
        if (settings.getBoolean("swithCPUFanSpeedYear", true)) {
        ...
        if(settings.getBoolean("swithSYSFanSpeedHour",true)) {
            item = new StatisticItem(DaignostiquesSystemeController.Diagnostique_sys_hour);
            mStatisticItemList.add(item);
        }
        if(settings.getBoolean("swithSYSFanSpeedDay",true)) {
        ...
        if(settings.getBoolean("swithSYSFanSpeedMonth",true)) {
            item = new StatisticItem(DaignostiquesSystemeController.Diagnostique_sys_month);
            mStatisticItemList.add(item);
        }
        if(settings.getBoolean("swithSYSFanSpeedYear",true)) {
        ...
        mNutController.getSettings(new CallbackImpl(StatisticsScrollingActivity.this) {

            @Override
            public void onResponse(Call call, Response response) throws IOException, InterruptedException {
                super.onResponse(call,response);
                final NutSettings settings =  response.GetResultObject(new TypeToken<NutSettings>(){});
                mHandler.post(new Runnable(){
                    public void run() {
                        //mOutputController.IsRunningFile(filename);
                        AddNut(settings);
    ...
    private void AddNut(NutSettings settings2) {
        Nutplugin = false;
        ...
        StatisticItem item;
            ...
            //Nut Charge
            if (settings.getBoolean("swithNutChargeHour", false)) {
                item = new StatisticItem(NutController.Nut_charge_hour);
                mStatisticItemList.add(item);
            }
            if (settings.getBoolean("swithNutChargeDay", false)) {
                item = new StatisticItem(NutController.Nut_charge_day);
                mStatisticItemList.add(item);
            }
            if (settings.getBoolean("swithNutChargeWeek", false)) {
                item = new StatisticItem(NutController.Nut_charge_week);
                mStatisticItemList.add(item);
            }
            if (settings.getBoolean("swithNutChargeMonth", false)) {
                item = new StatisticItem(NutController.Nut_charge_month);
                mStatisticItemList.add(item);
            }
            if (settings.getBoolean("swithNutChargeYear", false)) {
                item = new StatisticItem(NutController.Nut_charge_year);
                mStatisticItemList.add(item);
            }


            if (settings.getBoolean("swithNutloadHour", false)) {
                item = new StatisticItem(NutController.Nut_load_hour);
                mStatisticItemList.add(item);
            }
            if (settings.getBoolean("swithNutloadDay", false)) {
                item = new StatisticItem(NutController.Nut_load_day);
                mStatisticItemList.add(item);
            }
            if (settings.getBoolean("swithNutloadWeek", false)) {
                item = new StatisticItem(NutController.Nut_load_week);
                mStatisticItemList.add(item);
            }
            if (settings.getBoolean("swithNutloadMonth", false)) {
                item = new StatisticItem(NutController.Nut_load_month);
                mStatisticItemList.add(item);
            }
            if (settings.getBoolean("swithNutloadYear", false)) {
                item = new StatisticItem(NutController.Nut_load_year);
                mStatisticItemList.add(item);
            }


            if (settings.getBoolean("swithNutTemperatureHour", false)) {
                item = new StatisticItem(NutController.Nut_temperature_hour);
                mStatisticItemList.add(item);
            }
            if (settings.getBoolean("swithNutTemperatureDay", false)) {
                item = new StatisticItem(NutController.Nut_temperature_day);
                mStatisticItemList.add(item);
            }
            if (settings.getBoolean("swithNutTemperatureWeek", false)) {
                item = new StatisticItem(NutController.Nut_temperature_week);
                mStatisticItemList.add(item);
            }
            if (settings.getBoolean("swithNutTemperatureMonth", false)) {
                item = new StatisticItem(NutController.Nut_temperature_month);
                mStatisticItemList.add(item);
            }
            if (settings.getBoolean("swithNutTemperatureYear", false)) {
                item = new StatisticItem(NutController.Nut_temperature_year);
                mStatisticItemList.add(item);
            }


            if(settings.getBoolean("swithNutVoltageHour",false)) {
                item = new StatisticItem(NutController.Nut_voltage_hour);
                mStatisticItemList.add(item);
            }
            if(settings.getBoolean("swithNutVoltageDay",false)) {
                item = new StatisticItem(NutController.Nut_voltage_day);
                mStatisticItemList.add(item);
            }
            if(settings.getBoolean("swithNutVoltageWeek",false)) {
                item = new StatisticItem(NutController.Nut_voltage_week);
                mStatisticItemList.add(item);
            }
            if(settings.getBoolean("swithNutVoltageMonth",false)) {
                item = new StatisticItem(NutController.Nut_voltage_month);
                mStatisticItemList.add(item);
            }
            if(settings.getBoolean("swithNutVoltageYear",false)) {
                item = new StatisticItem(NutController.Nut_voltage_year);
                mStatisticItemList.add(item);
            }
    ...
    private void showDialogNut()
    {
        DialogFragment dialog = new FilterNutDialogFragment();
        dialog.show(getSupportFragmentManager(), "FilterNutDialogFragment");
    }
